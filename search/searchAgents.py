        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                if (nextx, nexty) in unvisitedCorners:
                    unvisitedCorners.remove((nextx, nexty))
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    tempUnvisitedCorners = unvisitedCorners.copy()

    sum = 0 
    while len(tempUnvisitedCorners) > 0:
        minDist = 999999
        minCorner = None
        for corner in tempUnvisitedCorners:
            distX = abs(position[0] - corner[0])
            distY = abs(position[1] - corner[1])
            dist = distX+distY
            if dist < minDist:
                minDist = dist
                minCorner = corner
        sum += minDist
        position = minCorner
        tempUnvisitedCorners.remove(minCorner)
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    #     dist = distX+distY
    #     dist = position, food, problem.startingGameState
    #     if dist < minDist:
    #         minDist = dist
    #         minDistToFood = food

    maxDistBetweenFood = 0
    for food in foodList:
        for food2 in foodList:
            distX = abs(food[0] - food2[0])
            distY = abs(food[1] - food2[1])
            dist = distX+distY
            if dist > maxDistBetweenFood:
                maxDistBetweenFood = dist

    sum  = sum + maxDistBetweenFood
    #  - minDistToFood
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        minFood = None
        for food in foodList:
            distX = abs(startPosition[0] - food[0])
            distY = abs(startPosition[1] - food[1])
            dist = distX+distY
            if dist < minDist:
                minDist = dist
                minFood = food
        ...
        if minFood != None:
            problem = PositionSearchProblem(gameState, start=startPosition, goal=minFood, warn=False, visualize=False)
