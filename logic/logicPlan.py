...
def sentence1() -> Expr:
    ...
    first = Expr("A") | Expr("B")
    second = ~Expr("A") % (~Expr("B") | Expr("C"))
    third = disjoin(~Expr("A"), ~Expr("B"), Expr("C"))
...
def sentence2() -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    first = Expr("C") % (Expr("B") | Expr("D"))
    second = Expr("A") >> (~Expr("B") & ~Expr("D"))
    third = ~(Expr("B") & ~Expr("C")) >> Expr("A")
    fourth = ~Expr("D") >> Expr("C")
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    isEntailed = premise & ~conclusion

    return findModel(isEntailed) is False
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    combosOfTwoLiterals = itertools.combinations(literals, 2)
    instersectionOfTwoLiterals = []
    for combo in combosOfTwoLiterals:
        instersectionOfTwoLiterals.append(~combo[0] | ~combo[1])
    return conjoin(instersectionOfTwoLiterals)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(
    ...
    for x, y in non_outer_wall_coords:
        inner_grid.append(PropSymbolExpr(pacman_str, x, y, time=t))
    pacphysics_sentences.append(exactlyOne(inner_grid))

    all_actions = []
    for dir in DIRECTIONS:
        all_actions.append(PropSymbolExpr(dir, time=t))
    pacphysics_sentences.append(exactlyOne(all_actions))

    if sensorModel != None:
        pacphysics_sentences.append(sensorModel(t, non_outer_wall_coords))

    # no need to check successor axioms if t=0
    if (successorAxioms != None) & (t != 0):
        pacphysics_sentences.append(
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for x, y in non_wall_coords:
            possible_pos_at_t.append(PropSymbolExpr(pacman_str, x, y, time=t))
        KB.append(exactlyOne(possible_pos_at_t))
        ...
        allLegalSuccessors = []
        for x, y in non_wall_coords:
            allLegalSuccessors.append(
                pacmanSuccessorAxiomSingle(x, y, t + 1, walls_grid)
            )

        KB.append(conjoin(allLegalSuccessors))
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        allLegalSuccessors = []
        for x, y in non_wall_coords:
            allLegalSuccessors.append(pacmanSuccessorAxiomSingle(x, y, t + 1, walls))
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        possible_locations = []
        for x, y in non_outer_wall_coords:
            entailsForPostiive = entails(
                conjoin(KB), PropSymbolExpr(pacman_str, x, y, time=t)
            )
            entailsForNegative = entails(
                conjoin(KB), ~PropSymbolExpr(pacman_str, x, y, time=t)
            )

            if entailsForPostiive:
                KB.append(PropSymbolExpr(pacman_str, x, y, time=t))
            elif entailsForNegative:
                KB.append(~PropSymbolExpr(pacman_str, x, y, time=t))

            if not entailsForNegative:
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        #  provable wall location

        for x, y in non_outer_wall_coords:
            entailsForPostiive = entails(conjoin(KB), PropSymbolExpr(wall_str, x, y))
            entailsForNegative = entails(conjoin(KB), ~PropSymbolExpr(wall_str, x, y))
